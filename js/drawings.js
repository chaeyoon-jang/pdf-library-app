...
$('#pagesContainer').addEventListener('pointerdown', e => {
  if (!drawMode) return;
  if (e.pointerType === 'touch') return;        // finger → let the page scroll
  ...
  if (drawTool === 'eraser') {
    ...
    V.anns.drawings = V.anns.drawings.filter(d =>
      !(d.page === pageNo && strokeHit(d, x, y, 0.005)));
    if (V.anns.drawings.length !== before) {
      paintDrawings(pageNo); renderSidebar(); scheduleSave();
    }
    return;
  }
...
$('#pagesContainer').addEventListener('pointerup', finishStroke);
$('#pagesContainer').addEventListener('pointercancel', finishStroke);
$('#pagesContainer').addEventListener('pointerleave', finishStroke);
